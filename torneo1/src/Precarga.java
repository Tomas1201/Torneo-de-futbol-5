...
public class Precarga {
    ...
    public void pre() {
        ...
        for (int i = 0; i < ins.getModalidad(); i++) {
            ins.getEquipos2()[i] = new Equipos();
            System.arraycopy(arrayEquipos, i, ins.getEquipos2(), i, 1);
            ins.getEquipos2()[i].setNombreEquipo("marcelo");
            ins.getEquipos2()[i].setCapitan(
                    nombrePrimero[generarNumeroAleatorio(0, 99)] + " " + apellido[generarNumeroAleatorio(0, 99)]);
            ins.getEquipos2()[i].setCantidadJugadores(generarNumeroAleatorio(9, 14));
            ins.getEquipos2()[i].setSubcapitan(
                    nombrePrimero[generarNumeroAleatorio(0, 99)] + " " + apellido[generarNumeroAleatorio(0, 99)]);
            for (int k = 0; k < ins.getEquipos2()[i].getJugadores2().length; k++) {
                ins.getEquipos2()[i].getJugadores2()[k].setNombreJugador(
                        nombrePrimero[generarNumeroAleatorio(0, 99)] + " " + apellido[generarNumeroAleatorio(0, 99)]);
                ins.getEquipos2()[i].getJugadores2()[k].setDNI(generarNumeroAleatorio(10000000, 40000000));
            }
        }
